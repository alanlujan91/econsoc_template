...
- **Numbered links to sections render `??`** when the numbering keys are set at project level in `myst.yml` rather than in the document's own frontmatter: `[Sec %s](#sec-one)` emits `Sec~??` and the PDF prints `Sec ??`, at exit 0 with nothing in the log. Plain `[](#sec-one)` and `@sec-one` still work, which is what makes it easy to miss. Cause and fix in [jupyter-book/mystmd#3035](https://github.com/jupyter-book/mystmd/pull/3035): a single-article export sets the article level to 0, so the numbering lookup asks for a `heading_0` key that no configuration can enable. Until it releases, set `title: true` and `headings: true` in the document's frontmatter, as the sample does, or write the reference as raw LaTeX. `scripts/build-samples.sh` fails the build on any `??` in a rendered PDF.
